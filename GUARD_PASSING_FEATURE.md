...
```bash
curl -X POST https://fightscoreai.preview.emergentagent.com/api/events/v2/log \
...
```bash
curl -X POST https://fightscoreai.preview.emergentagent.com/api/calculate-score-v2 \
  -H "Content-Type: application/json" \
  -d '{
    "bout_id": "bout-123",
    "round_num": 1,
