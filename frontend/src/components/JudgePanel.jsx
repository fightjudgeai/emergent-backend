...
export default function JudgePanel() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] p-4 md:p-8">
      ...
      {/* At-a-Glance Stats - Fighter Comparison */}
      {Object.keys(scores).length > 0 && (
        <div className="max-w-7xl mx-auto mb-6">
          <Card className="bg-gradient-to-br from-[#1a1d24] to-[#13151a] border-amber-500/30 overflow-hidden">
            <div className="bg-gradient-to-r from-amber-600/20 to-orange-600/20 border-b border-amber-500/30 px-6 py-3">
              <h2 className="text-lg font-bold text-amber-400 flex items-center gap-2">
                <TrendingUp className="h-5 w-5" />
                At-a-Glance Fight Statistics
              </h2>
            </div>
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    Object.values(scores).forEach(s => {
                      if (s && s.fighter1_score && s.fighter1_score.event_counts) {
                        const counts = s.fighter1_score.event_counts;
                        totalStrikes += counts['Significant Strikes'] || 0;
                        totalTakedowns += counts['Takedowns'] || 0;
                        totalDamage += counts['Damage'] || 0;
                        totalSubs += counts['Submission Attempts'] || 0;
                      }
                    });
                    return (
                      <>
                        <div className="flex justify-between items-center px-3 py-2 bg-red-950/30 rounded">
                          <span className="text-sm text-gray-300">Total Strikes</span>
                          <span className="text-lg font-bold text-red-400">{totalStrikes}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-red-950/30 rounded">
                          <span className="text-sm text-gray-300">Takedowns</span>
                          <span className="text-lg font-bold text-red-400">{totalTakedowns}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-red-950/30 rounded">
                          <span className="text-sm text-gray-300">Damage Events</span>
                          <span className="text-lg font-bold text-red-400">{totalDamage}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-red-950/30 rounded">
                          <span className="text-sm text-gray-300">Sub Attempts</span>
                          <span className="text-lg font-bold text-red-400">{totalSubs}</span>
                        </div>
                      </>
                    );
                  })()}
              ...
              <div className="p-6 bg-gradient-to-br from-blue-900/10 to-transparent">
                ...
                <div className="space-y-3">
                  {(() => {
                    let totalStrikes = 0, totalTakedowns = 0, totalDamage = 0, totalSubs = 0, totalControlTime = 0;
                    Object.values(scores).forEach(s => {
                      if (s && s.fighter2_event_counts) {
                        totalStrikes += s.fighter2_event_counts['Significant Strikes'] || 0;
                        totalTakedowns += s.fighter2_event_counts['Takedowns'] || 0;
                        totalDamage += s.fighter2_event_counts['Damage'] || 0;
                        totalSubs += s.fighter2_event_counts['Submission Attempts'] || 0;
                      }
                    });
                    return (
                      <>
                        <div className="flex justify-between items-center px-3 py-2 bg-blue-950/30 rounded">
                          <span className="text-sm text-gray-300">Total Strikes</span>
                          <span className="text-lg font-bold text-blue-400">{totalStrikes}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-blue-950/30 rounded">
                          <span className="text-sm text-gray-300">Takedowns</span>
                          <span className="text-lg font-bold text-blue-400">{totalTakedowns}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-blue-950/30 rounded">
                          <span className="text-sm text-gray-300">Damage Events</span>
                          <span className="text-lg font-bold text-blue-400">{totalDamage}</span>
                        </div>
                        <div className="flex justify-between items-center px-3 py-2 bg-blue-950/30 rounded">
                          <span className="text-sm text-gray-300">Sub Attempts</span>
                          <span className="text-lg font-bold text-blue-400">{totalSubs}</span>
                        </div>
                      </>
                    );
                  })()}
